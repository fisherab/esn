...
public class DownloadServlet extends HttpServlet {
	...
	public void init() {
		try {
			Properties.check();
			File cacheBaseDir = Properties.getCacheBaseDir();
			File[] files = cacheBaseDir.listFiles();
			if (files == null) {
				Files.createDirectories(cacheBaseDir.toPath());
			} else {
				for (File f : files) {
					f.delete();
				}
	...
	Map<String, Integer> map = new LinkedHashMap<String, Integer>(MAX_ENTRIES,
			(float) 0.7, true) {
		...
		protected boolean removeEldestEntry(Map.Entry<String, Integer> eldest) {
			if (eldest.getValue() == 0) {
				...
				if (remove) {
					...
					File f = new File(Properties.getCacheBaseDir(),
							eldest.getKey());
					f.delete();
				}
				return remove;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse response)
		...
		try {
			...
			if (!f.exists()) {
				throw new IOException("Requested file " + name
						+ " does not exist");
			}
			...
			int reqWidth = reqWidthString == null ? 0 : Integer
					.parseInt(reqWidthString);
			int reqHeight = reqHeightString == null ? 0 : Integer
					.parseInt(reqHeightString);

			String key = name.replace("/", ",") + " " + reqWidth + "x"
					+ reqHeight;
			Integer entry = null;
			...
			while (true) {
				synchronized (map) {
					while (map.size() > MAX_ENTRIES) {
						...
						map.remove(keyToGo);
						logger.debug("Remove entry for " + keyToGo
								+ " as map has become too big.");
					}

					entry = map.get(key);
					if (entry != null) {
						if (entry >= 0) {
							map.put(key, entry + 1); // Increase current use
														// count
			...
			if (needsCacheEntry) {
				...
				float scale = 0;
				if (reqWidthString != null & reqHeightString != null) {
					scale = Math.max(image.getHeight() / (float) reqHeight,
							image.getWidth() / (float) reqWidth);
				...
				try {
					...
					logger.debug("Scaling " + name + " down by " + scale
							+ " to " + reqWidth + "x" + reqHeight);

					BufferedImage newImage = new BufferedImage(reqWidth,
							reqHeight, image.getType());
					...
					ImageOutputStream ios = ImageIO
							.createImageOutputStream(new File(Properties
									.getCacheBaseDir(), key));
